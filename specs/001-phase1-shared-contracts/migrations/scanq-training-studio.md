# Migration Runbook: scanq-training-studio → scanq-shared Phase 1

**Consumer**: `scanq-training-studio`
...
The following local contract definitions in `scanq-training-studio` are to be replaced with shared imports:
...
| Local `LinageEventType` (if present) | `LineageEventType` | `scanq_shared.enums` |
...
from training_studio.contracts import ContextResolveRequest, ContextResolveResponse
from training_studio.contracts import ServiceTokenRequest, ServiceTokenResponse
from training_studio.contracts import LineageRegisterRequest, LineageRegisterResponse
...
2. Search for local contract definitions: `grep -r "ContextResolveRequest\|ServiceTokenRequest\|LineageRegisterRequest" src/`
...
| Check | Result | Notes |
|---|---|---|
| Imports resolve without errors | **PASS** | Verified via `uv run python -c "from scanq_shared.schemas import ContextResolveRequest, ServiceTokenRequest, LineageRegisterRequest, LineageRegisterResponse; print('OK')"` |
| Phase 1 schema round-trip tests pass | **PASS** | `uv run pytest tests/contract/ tests/integration/ -q` — 75 passed |
| Shared package importable | **PASS** | `from scanq_shared.clients import TrainingStudioClient` resolves correctly |
| No consumer-side changes applied yet | N/A | Consumer repo `scanq-training-studio` not in scope of this workspace |

> **Note**: Full consumer-side dry-run (replacing local imports in `scanq-training-studio`) must be performed in that repository. Use this runbook's import replacement mapping as the migration guide.
...
- `scanq-training-studio` may still ship with local contract definitions **and** `scanq-shared` as a dependency.
- Both import paths must resolve without conflict.
- Verify: `python -c "from scanq_shared.schemas import ContextResolveRequest; print('OK')"` passes.
